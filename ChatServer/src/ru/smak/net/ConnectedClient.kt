        ...
        when (request["operation"]) {
            "LOGIN" -> {
                if (!name.isNullOrEmpty()) {
                    sendErrorResponse("LOGIN", "Вы уже вошли в аккаунт '$name'")
                    return
                }
                ...
                if (userEntity!!.password == password) {
                    ...
                    sendToAllConnectedClients( {"USERLOGGED"}, {
                        if (it != this)
                            "Пользователь $name зашел в чат! Жаль."
                        else {
            ...
            "SIGNUP" -> {
                ...
                sendToAllConnectedClients({ "USERSIGNIN" }, {
                    if (it != this)
                        "Пользователь $name впервые в нашем чате! Пусть этот раз будет последним."
                    else {
            ...
            "MESSAGE" -> {
                ...
                sendToAllConnectedClients({ "MESSAGE" }, { "${if (it == this) "Вы" else name}: $formattedMessage" })
            }

            "EXIT" -> {
                sendToAllConnectedClients({ "EXIT" }, {
                    if (it != this) {
                        "Пользователь $name вышел из чата. Туда ему и дорога."
                    } else {
                        "Вы вышли из чата."
                    }
                })
